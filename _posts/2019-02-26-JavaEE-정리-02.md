...
4. 대기
5. 웹브라우저 실행, URL 입력
6. 접속자가 접속하면서 생성된 Socket으로부터 RequestFacade, ResponseFacade가 생성
7. 요청을 처리하는 서블릿 객체 생성
8. 요청 방식(GET/POST)에 맞는 method 호출
9. HttpServletResponse 변수를 이용 MIME Type 설정, ResponseFacade로부터 스트림을 얻어 응답할 내용 누적
10. HTML을 응답, 클라이언트는 응답받은 HTML을 웹브라우저 그림
