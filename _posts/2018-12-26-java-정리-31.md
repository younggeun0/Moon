...
* **Java는 Multi Thread 지원**
    * DOS는 Single Thread,cmd에서 eclipse.bat을 실행하면 eclipse를 종료할 때까지 다른 작업을 못 한다.
* **Thread 사용된 코드는 종료시간을 측정할 수 없다.**
* **Runnable interface 구현, Thread class로 Thread 사용가능**
* **선점형 Thread**
    * **작업관리자가 필요없음**
        * **간단하게 구현되고 사용가능**
