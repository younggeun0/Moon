...
#### 함수
* 자주 사용될 코드를 미리 정의해두고 필요한 곳에서 호출하여 사용
* 조회하는 컬럼, WHERE 절, INSERT 구문에서 사용 가능 
* 문자열 함수
  - LENGTH(), UPPER(), LOWER(), INSTR(), SUBSTRING(), CONCAT(), INITCAP(), TRIM(), LTRIM(), RTRIM(), LPAD(), RPAD()
* 수학 함수
  - SIN(), COS(), TAN(), ROUND(), CEIL(), FLOOR(), TRUNC(), ABS()
* 변환 함수
