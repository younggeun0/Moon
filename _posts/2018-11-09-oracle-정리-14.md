...
tags: [오라클DBMS]
excerpt: "ORACLE DBMS공부정리 - PL/SQL PROCEDURE"
date: 2018-11-09
comments: true
---
...
--    i_dong이란 이름의 커서로 인식 error!
    WHERE dong LIKE i_dong||'%';  -- 이런식으로 사용!
```
...
BEGIN
    OPEN 커서명 FOR 쿼리문정의;
    -- 프로시저안에서 절대 인출(FETCH)하면 안됨, 인출시 밖에서 받아갈 수 없어짐!
END;
/
```
...
```sql
-- 프로시저 내부에서 FETCH 후 외부에서 커서를 출력하면 당연히 결과가 안나온다(No Records)
EXEC select_all_dept(:cur_dept);
PRINT cur_dept;
```
...
[숙제풀이](https://github.com/younggeun0/SSangYoung/blob/master/dev/query/date181108_pl/homework1.sql)