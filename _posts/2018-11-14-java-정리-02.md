...
// 작은바이트타입과 큰바이트타입이 연산되면 작은바이트타입이 큰바이트타입으로 형변환 후 연산
// byte + byte = int
...
// 정수는 그냥 int로 변수를 생성하면 걱정없음.. 메모리 마구마구 쓰세요~
// 메모리를 절약하기 위해 형변환을 여러번 하는 것보다 int로 연산하는게 더 빠르다! 
```
